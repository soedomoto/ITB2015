    ...
    private int sync() {
        ...
        try {
            ...
            for(MWilayahCacah wilcah : wilcahs) {
                ...
                ContentResponse r7 = _client.GET(_properties.getRealHost() + _properties.getContextPath() + CPencacah.Pencacah.PATH +
                        "?kode=" + wilcah.getPencacah().getId());
                MPencacah pencacah = _gson.fromJson(r7.getContentAsString(), MPencacah.class);

                MPencacah pchL = _dao.pencacahDao().queryForId(pencacah.getId());
                if(pchL != null) {
                    if(pencacah.getLastUpdate().after(pchL.getLastUpdate())) {
                        changes += _dao.pencacahDao().update(pencacah);
                    }
                } else {
                    changes += _dao.pencacahDao().create(pencacah);
                }

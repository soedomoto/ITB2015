...
public class CFormL1 {
    private static CFormL1 cFormL1;
    public static CFormL1 instance() {
        if(cFormL1 == null) cFormL1 = new CFormL1();
        return cFormL1;
    }

    private Dao<MFormL1, String> formL1Dao;
    private Dao<MFormL1B5, String> formL1B5Dao;
    private Dao<MFormL1B5Usaha, String> formL1B5UsahaDao;
    ...
    public void registerServlets(ServletContextHandler context) {
        ServletHolder dataL1 = new ServletHolder(new DataL1());
        dataL1.setAsyncSupported(true);
        context.addServlet(dataL1, DataL1.PATH);

        ServletHolder submit = new ServletHolder(new Submit());
        submit.setAsyncSupported(true);
        context.addServlet(submit, Submit.PATH);

        ServletHolder sync = new ServletHolder(new Sync());
        sync.setAsyncSupported(true);
        context.addServlet(sync, Sync.PATH);
    }

    public Dao<MFormL1, String> getFormL1Dao() {
        return formL1Dao;
    }

    public Dao<MFormL1B5, String> getFormL1B5Dao() {
        return formL1B5Dao;
    }

    public Dao<MFormL1B5Usaha, String> getFormL1B5UsahaDao() {
        return formL1B5UsahaDao;
    }
    ...
    public static class DataL1 extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
            ...
            final String kodeBlokSensus = req.getParameter("bs");
                    ...
                    try {
                        try {
                            ...
                            if(kodePencacah != null) {
                                ...
                                l1s = CFormL1.instance().getFormL1Dao().queryForMatching(new MFormL1(bs));
                            } else {
                                l1s = CFormL1.instance().getFormL1Dao().queryForAll();
                            }

                            for(MFormL1 l1 : l1s) {
                                List<MFormL1B5> b5s = CFormL1.instance().getFormL1B5Dao().queryForMatching(new MFormL1B5(l1));
                                for(MFormL1B5 b5 : b5s) {
                                    List<MFormL1B5Usaha> usahaas = CFormL1.instance().getFormL1B5UsahaDao().queryForMatching(new MFormL1B5Usaha(b5));
                                    b5.setUsaha(usahaas);
                                }
                ...
                public void run() {
                    try {
                        try {
                            ...
                            for(MFormL1 l1 : l1s) {
                                MFormL1 l1L = CFormL1.instance().getFormL1Dao().queryForId(l1.getFullKode());
                                if(l1L != null) {
                                    if(l1.getLastUpdate().after(l1L.getLastUpdate())) {
                                        changes += CFormL1.instance().getFormL1Dao().update(l1);
                                    }
                                } else {
                                    changes += CFormL1.instance().getFormL1Dao().create(l1);
                                }

                                for(MFormL1B5 b5 : l1.getB5()) {
                                    MFormL1B5 b5L = CFormL1.instance().getFormL1B5Dao().queryForId(b5.getFullKode());
                                    if(b5L != null) {
                                        if(b5.getLastUpdate().after(b5L.getLastUpdate())) {
                                            changes += CFormL1.instance().getFormL1B5Dao().update(b5);
                                        }
                                    } else {
                                        changes += CFormL1.instance().getFormL1B5Dao().create(b5);
                                    }

                                    for(MFormL1B5Usaha usaha : b5.getUsaha()) {
                                        MFormL1B5Usaha usahaL = CFormL1.instance().getFormL1B5UsahaDao().queryForId(usaha.getFullKode());
                                        if(usahaL != null) {
                                            if(usaha.getLastUpdate().after(usahaL.getLastUpdate())) {
                                                changes += CFormL1.instance().getFormL1B5UsahaDao().update(usaha);
                                            }
                                        } else {
                                            changes += CFormL1.instance().getFormL1B5UsahaDao().create(usaha);
                                        }
                                    }
                                }
                            }
    ...
    public static class Submit extends HttpServlet {
        ...
        private void _saveRecord(FormL1 jsonFormL1, HttpServletResponse resp) throws IOException {
            try {
                MPropinsi v101              = CPropinsi.instance().getV101Dao().queryForId(jsonFormL1.v101);
                MKabupaten v102             = CKabupaten.instance().getV102Dao().queryForId(jsonFormL1.v101 + jsonFormL1.v102);
                MKecamatan v103             = CKecamatan.instance().getV103Dao().queryForId(jsonFormL1.v101 + jsonFormL1.v102 +
                        jsonFormL1.v103);
                MKelurahan v104             = CKelurahan.instance().getV104Dao().queryForId(jsonFormL1.v101 + jsonFormL1.v102 +
                        jsonFormL1.v103 + jsonFormL1.v104);
                MBlokSensus v105            = CBlokSensus.instance().getV105Dao().queryForId(jsonFormL1.v101 + jsonFormL1.v102 +
                        jsonFormL1.v103 + jsonFormL1.v104 + jsonFormL1.v105);
                MSubBlokSensus v106         = CSubBlokSensus.instance().getV106Dao().queryForId(jsonFormL1.v101 + jsonFormL1.v102 +
                        jsonFormL1.v103 + jsonFormL1.v104 + jsonFormL1.v105 +
                        jsonFormL1.v106);
                MNks v107                   = CNks.instance().getV107Dao().queryForId(jsonFormL1.v107);
                MSls v108                   = CSls.instance().getV108Dao().queryForId(jsonFormL1.v101 + jsonFormL1.v102 +
                ...
                CFormL1.instance().getFormL1Dao().createOrUpdate(formL1);
                ...
                for(FormL1B5 b5 : jsonFormL1.b5) {
                    ...
                    CFormL1.instance().getFormL1B5Dao().createOrUpdate(formL1B5);
                    ...
                    for(FormL1B5Usaha b5Usaha : b5.usaha) {
                        ...
                        CFormL1.instance().getFormL1B5UsahaDao().createOrUpdate(formL1B5Usaha);
                    }
                }

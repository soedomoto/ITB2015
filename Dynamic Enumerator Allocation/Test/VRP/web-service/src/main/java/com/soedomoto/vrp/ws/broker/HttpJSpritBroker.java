 ...
 */
public class HttpJSpritBroker extends HttpAbstractBroker implements Runnable {
    ...
    public HttpJSpritBroker(ScheduledExecutorService executor, ServletContext context) {
    ...
    public void run() {
        ...
        try {
            LOG.debug(String.format("Processing subscribe(s) from %s", asyncResponseMap.keySet()));
            ...
            for(Subscriber s : workingSubscribers) {
                if(routeMap.keySet().contains(s.getSubscriber())) {
                    ...
                    try {
                        ...
                        String strCurrPath = null;
                        try {
                            strCurrPath = new ObjectMapper().writeValueAsString(currPath);
            ...
            for(long rid : asyncResponseMap.keySet()) {
                asyncResponseMap.get(rid).cancel();
                asyncResponseMap.remove(rid);
            }
